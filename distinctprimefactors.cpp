...
/**************************************************************  	
  	          Main Code
**************************************************************/
const ll max1=2000000+7 ;
ll n,k ;
ll sieve[max1],ds[max1] ;
...
int main()
{
    ...
    for(ll i=2;i<max1;i++)
    {
    	if(!sieve[i])
    	{
    		for(ll j=i;j<max1;j=j+i)
    		{
    			sieve[j]=i ;
    		}
    	}
    }
    //cout << no_of_distinctprimes(644) ;
    for(ll i=2;i<n+k;i++)
    {
    	ds[i] = ds[i-1] + (no_of_distinctprimes(i)==k) ;
    	if(i>=k && ds[i]-ds[i-k]>=k)
    	{
    		cout << i-k+1 << "\n" ;
    	}
    }
