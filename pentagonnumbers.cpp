...
ll check(double n)
{
	double ans = (1+sqrt(1+24*n))/6 ;
	//cout << ans << "this is calculated value\n" ;
	//cout << int(ans) << "this is answer after typecasting" ;
	if(int(ans)==ans)
	{
		return 1 ;
	} 
...
int main()
{
    ...
    {
    	double ans = (i*(3*i-1))/2 ;
    	v.PB(ans) ;
    }
    for(double i=1+k;i<n;i++)
    {
    	if(check(v[i]-v[i-k])!=0 || check(v[i]+v[i-k])!=0)
    	{
    		ans.PB(v[i]) ;
    	}
    }
